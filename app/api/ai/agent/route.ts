...
Return the JSON with the required changes.`
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        send({ type: "thinking", content: `📋 Reading spreadsheet (${numRows} rows × ${numCols} cols)` })
        ...
        for await (const chunk of streamResult.textStream) {
          ...
          if (!sentPlanningMsg && fullText.length > 10) {
            send({ type: "thinking", content: "🧠 Analyzing request and planning changes..." })
            sentPlanningMsg = true
          }
          if (!sentWritingMsg && fullText.length > 150) {
            send({ type: "thinking", content: "✏️ Generating cell changes..." })
            sentWritingMsg = true
          }
        }
