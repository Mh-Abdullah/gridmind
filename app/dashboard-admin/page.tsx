        ...
        <main className="flex-1 overflow-auto p-4 md:p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-card border border-border rounded-lg p-6">
              ...
              <p className="text-3xl font-bold text-foreground">{users.filter((item) => item.role === "admin").length}</p>
            </div>
            <div className="bg-card border border-border rounded-lg p-6">
                  ...
                  <tbody>
                    {users.map((item) => (
                      <tr key={item.id} className="border-b border-border hover:bg-muted/50 transition">
                        <td className="px-6 py-4 text-sm text-foreground font-medium">{item.name || "N/A"}</td>
                        <td className="px-6 py-4 text-sm text-muted-foreground">{item.email}</td>
                        <td className="px-6 py-4 text-sm">
                          <span
                            className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${
                              item.role === "admin" ? "bg-muted text-foreground" : "bg-muted text-muted-foreground"
                            }`}
                          >
