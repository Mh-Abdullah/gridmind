...
export function Card2({
  bars = [45, 75, 35, 85, 60, 95, 50],
  metrics = [
    { label: "Tokens/min", value: "12.4k", trend: "+8%" },
    { label: "Cost/run", value: "$0.042", trend: "-3%" },
  ],
}: Card2Props) {
  const days = ["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"].slice(0, bars.length);
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3.5 justify-between">
      ...
      <div className="flex flex-col gap-3 pt-[0.625rem] pr-[0.625rem] pb-0.5 pl-0.5">
        {metrics.slice(0, 2).map((s, i) => {
          const isActive = i === activeIdx || hoveredIdx === i;

          return (
            <div key={i} className="h-[76px] w-full relative select-none">
