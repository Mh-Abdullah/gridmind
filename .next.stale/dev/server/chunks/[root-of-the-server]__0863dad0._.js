...
"[project]/app/api/ai/agent/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    ...
    const dataLines = tableRows.map((row, r)=>`Row${String(r + 1).padStart(2)} | ${row.map((v)=>(v || "").toString().slice(0, 20).padEnd(20)).join(" | ")}`);
    const spreadsheetText = [
        headerLine,
        separator,
        ...dataLines
    ].join("\n");
    const recentHistory = (chatHistory || []).filter((m)=>m.role === "user" || m.role === "assistant").slice(-6).map((m)=>`${m.role === "user" ? "User" : "Assistant"}: ${m.content.slice(0, 300)}`).join("\n");
    ...
    if (selectedCells && selectedCells.length > 0) {
        ...
        const selectedRows = [
            ...new Set(selectedCells.map((s)=>s.row))
        ];
    ...
    const stream = new ReadableStream({
        async start (controller) {
            ...
            try {
                send({
                    type: "thinking",
                    content: `📋 Reading spreadsheet (${numRows} rows × ${numCols} cols)`
                });
                const streamResult = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["streamText"])({
                ...
                let fullText = "";
                let sentPlanningMsg = false;
                let sentWritingMsg = false;
                for await (const chunk of streamResult.textStream){
                    fullText += chunk;
                    if (!sentPlanningMsg && fullText.length > 10) {
                        send({
                            type: "thinking",
                            content: "🧠 Analyzing request and planning changes..."
                        });
                        sentPlanningMsg = true;
                    }
                    if (!sentWritingMsg && fullText.length > 150) {
                        send({
                            type: "thinking",
                            content: "✏️ Generating cell changes..."
                        });
                        sentWritingMsg = true;
                    }
                }
                console.log("[Agent API] Streamed response length:", fullText.length);
                // Parse JSON from streamed response
                let agentData = null;
                try {
                    const codeBlock = fullText.match(/```json\s*([\s\S]*?)```/);
                    if (codeBlock) agentData = JSON.parse(codeBlock[1].trim());
                    if (!agentData) {
                        const jsonMatch = fullText.match(/\{[\s\S]*\}/);
                        if (jsonMatch) agentData = JSON.parse(jsonMatch[0]);
                    }
                    if (!agentData) agentData = JSON.parse(fullText.trim());
                } catch (e) {
                ...
                if (!agentData || !Array.isArray(agentData.changes) && !Array.isArray(agentData.formatting)) {
                    ...
                    const c = (agentData.changes || []).length;
                    const f = (agentData.formatting || []).length;
                    const parts = [];
                    if (c > 0) parts.push(`${c} cell edit${c !== 1 ? 's' : ''}`);
                    if (f > 0) parts.push(`${f} formatting change${f !== 1 ? 's' : ''}`);
                    if (parts.length > 0) {
                        send({
                            type: "thinking",
                            content: `✅ Prepared: ${parts.join(' · ')}`
                        });
                    }
                    send({
                        type: "result",
                ...
                send({
                    type: "error",
                    content: error instanceof Error ? error.message : "Failed to process agent request"
                });
            } finally{
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0863dad0._.js.map